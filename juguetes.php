<?php
// Página de juguetes: muestra productos de la categoría juguetes
...
// Obtener productos de la categoría juguetes 
$stmt = $pdo->prepare("SELECT * FROM productos WHERE id_tipo = ?");
$stmt->execute([4]);
$productos_juguetes = $stmt->fetchAll();

// Crear objetos Juguete
$juguetes = [];
foreach ($productos_juguetes as $prod) {
    $stmt_tipo = $pdo->prepare("SELECT nombre FROM tipos_productos WHERE id_tipo = ?");
    $stmt_tipo->execute([$prod['id_tipo']]);
    $nombre_tipo = $stmt_tipo->fetchColumn() ?: 'Juguete';
    $tipo = new TipoProducto($prod['id_tipo'], $nombre_tipo);
    $juguetes[] = new Juguete($prod['id_producto'], $prod['nombre'], $prod['precio'], $prod['imagen'], $tipo);
}
?>
<!DOCTYPE html>
            ...
            <div class="carousel-track">
                <?php foreach ($juguetes as $juguete): ?>
                    <form class="producto-item" method="post" action="listaDeseos.php" style="max-width:340px;min-width:260px;">
                        <input type="hidden" name="producto_imagen" value="<?php echo htmlspecialchars($juguete->imagen); ?>">
                        <input type="hidden" name="producto_nombre" value="<?php echo htmlspecialchars($juguete->nombre); ?>">
                        <img src="<?php echo htmlspecialchars($juguete->imagen); ?>" alt="<?php echo htmlspecialchars($juguete->nombre); ?>" style="width:100%;height:220px;object-fit:contain;">
                        <p class="nombre-producto"><?php echo htmlspecialchars($juguete->nombre); ?></p>
                        <div style="display:flex;justify-content:center;gap:1.2rem;align-items:center;">
                            ...
                            <button class="add-to-cart" title="Añadir al carrito"
                                data-id="<?php echo $juguete->id_producto; ?>"
                                data-nombre="<?php echo htmlspecialchars($juguete->nombre); ?>"
                                data-precio="<?php echo htmlspecialchars($juguete->precio); ?>"
