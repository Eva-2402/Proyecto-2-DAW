<?php
// Página de comederos: muestra productos de comederos y permite añadir a deseos o carrito
...
// lista de productos de comederos
...
// Obtener productos de la categoría comederos 
$stmt = $pdo->prepare("SELECT * FROM productos WHERE id_tipo = ?");
$stmt->execute([2]);
$productos_cuencos = $stmt->fetchAll();

// Crear objetos Comedero
$comederos = [];
foreach ($productos_cuencos as $prod) {
    $stmt_tipo = $pdo->prepare("SELECT nombre FROM tipos_productos WHERE id_tipo = ?");
    $stmt_tipo->execute([$prod['id_tipo']]);
    $nombre_tipo = $stmt_tipo->fetchColumn() ?: 'Comedero';
    $tipo = new TipoProducto($prod['id_tipo'], $nombre_tipo);
    $comederos[] = new Comedero($prod['id_producto'], $prod['nombre'], $prod['precio'], $prod['imagen'], $tipo);
}
?>
<!DOCTYPE html>
...
<body <?php echo get_body_attrs(); ?>>
    ...
    <div class="mini-deseos">
        <?php
        // Contamos cuántos productos hay en la lista, usando un array vacío si no existe
    ...
    <!-- Listado de productos de comederos -->
    ...
    <div class="carrusel-contenedor">
        ...
        <div class="carousel">
            <div class="carousel-track">
                <?php foreach ($comederos as $comedero): ?>
                    <form class="producto-item" method="post" action="listaDeseos.php" style="max-width:340px;min-width:260px;">
                        <input type="hidden" name="producto_imagen" value="<?php echo htmlspecialchars($comedero->imagen); ?>">
                        <input type="hidden" name="producto_nombre" value="<?php echo htmlspecialchars($comedero->nombre); ?>">
                        <img src="<?php echo htmlspecialchars($comedero->imagen); ?>" alt="<?php echo htmlspecialchars($comedero->nombre); ?>" style="width:100%;height:220px;object-fit:contain;">
                        <p class="nombre-producto"><?php echo htmlspecialchars($comedero->nombre); ?></p>
                        <div style="display:flex;justify-content:center;gap:1.2rem;align-items:center;">
                            ...
                            <button class="add-to-cart" title="Añadir al carrito"
                                data-id="<?php echo $comedero->id_producto; ?>"
                                data-nombre="<?php echo htmlspecialchars($comedero->nombre); ?>"
                                data-precio="<?php echo htmlspecialchars($comedero->precio); ?>"
